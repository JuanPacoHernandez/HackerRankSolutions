...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan  3 10:58:21 2023
...
def minimumDistances(a):
    if len(set(a)) == len(a):
        ...
        dicc = Counter(a)
        # DELETING THE KEYS NO NEEDED ANYMORE
        for k, v in list(dicc.items()):
            if v < 2:
                del dicc[k]
        # GETTING ALL INDICES OF NUMBERS WITH FREQUENCY GREATER THAN 2
        indices = []
        for j in list(dicc.keys()):
            s = [i for i in range(len(a)) if a[i] == j]
            indices.append(s)
        # FINDING DISTANCES BETWEEN THE INDICES OBTAINED PREVIOULSY
        distances = []
        for i in indices:
            if len(i) == 2:
            ...
            if len(i) > 2:
                for j in combinations(i,2):
                    distances.append(j[1] - j[0])
        # THE RESULT IS THE MINIMUM DISTANCE FROM LIST OF DISTANCES
